...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False, knn=False):
    ...
    # TODO: add relative positional embedding here
    if returnfps:
        return new_xyz, new_points, grouped_xyz, fps_idx
    else:
        return new_xyz, new_points

def sample_and_group_by_clusters(npoint, radius, nsample, xyz, points, returnfps=False, knn=False, cluster_number=16):
    B, N, C = xyz.shape
    S = npoint
    fps_idx = farthest_point_sample(xyz, npoint) # [B, npoint]
    torch.cuda.empty_cache()
...
class PointNetSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        dists = square_distance(new_xyz, new_xyz)
        knn_idx = dists.argsort()[:, :, :self.nsample]  # b x n x k
        knn_xyz = index_points(new_xyz, knn_idx)
...
class PointNetSetAbstractionRelPos(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all, knn=False):
        super(PointNetSetAbstractionRelPos, self).__init__()
        self.npoint = npoint
        self.radius = radius
        self.nsample = nsample
        self.knn = knn
    ...
    def forward(self, xyz, points):
        ...
        for i, conv in enumerate(self.mlp_convs):
            bn = self.mlp_bns[i]
            new_points =  F.relu(bn(conv(new_points + self.pos_embeds[i](new_xyz[:, :, None] - knn_xyz).permute(0,3,2,1))))
            #new_points =  F.relu(bn(conv(new_points)))
        #new_points = torch.max(new_points + self.last_pos_embed(new_xyz[:, :, None] - knn_xyz).permute(0,3,2,1), 2)[0].transpose(1, 2)
        new_points = torch.max(new_points, 2)[0].transpose(1, 2)
